...
#' @examples
#' @export
quantile_optim_DDM <- function(params, observations, returnFit){
  ...
  for (d in drift) {
    predictions <- data.frame(DDM_with_confidence_slow(v=d,a=params['a'],ter=params['ter'],z=params['z'],ntrials=params['ntrials']/2,s=params['sigma'],dt=params['dt'],t2time=params['t2time'],postdriftmod=params['vratio']))
    predictionsneg <- data.frame(DDM_with_confidence_slow(v=-d,a=params['a'],ter=params['ter'],z=params['z'],ntrials=params['ntrials']/2,s=params['sigma'],dt=params['dt'],t2time=params['t2time'],postdriftmod=params['vratio']))
  ...
  if(returnFit==0){
    ...
    difflevels <- sort(unique(observations$trialdifflevel)) #/!\ CHANGE ACCORDING TO DATASET
    ...
    for (d in 1:length(drift)) {
      ...
      c_quantiles <- quantile(c_observed[c_observed$trialdifflevel == difflevels[d],]$rt, probs = c(.1,.3,.5,.7,.9), names = FALSE)
      e_quantiles <- quantile(e_observed[e_observed$trialdifflevel == difflevels[d],]$rt, probs = c(.1,.3,.5,.7,.9), names = FALSE)
      ...
      prop_obs_c <- dim(c_observed[c_observed$trialdifflevel == difflevels[d],])[1] / dim(observations)[1]
      prop_obs_e <- dim(e_observed[e_observed$trialdifflevel == difflevels[d],])[1] / dim(observations)[1]
...
#' @examples
#' @export
quantile_optim_DDM_fullconfRT <- function(params, observations, returnFit,confRT_name = "RTconf"){
  #First, generate predictions:
  ...
  for (d in 1:length(drift)) {
    predictions <- data.frame(DDM_with_confidence_slow_fullconfRT(
      ...
      dt=params['dt'],t2distribution=rep(observations[observations$trialdifflevel==difflevels[d],confRT_name],times=params['ntrials']),
