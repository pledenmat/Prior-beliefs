...
trialdifflevel <- sort(unique(Data_exp1$trialdifflevel));
...
for(i in 1:Nsub_1){
  ...
  file_ddm <- paste0('Exp1/quantitative_fit/ddm/testfit',subs1[i],'.Rdata')
  file_conf <- paste0('Exp1/quantitative_fit/v_s_biassub/testfit',subs1[i],'.Rdata')
  file_conf_bias <- paste0('Exp1/quantitative_fit/bias/testfit',subs1[i],'.Rdata')
  file_conf_full <- paste0('Exp1/quantitative_fit/v_s_bias/testfit',subs1[i],'.Rdata')
  file_ok <- all(file.exists(file_conf,file_ddm,file_conf_bias,file_conf_full))
  if(file_ok){
    load(file_ddm)
...
params <- rbind(params,params_bias,params_full)
...
for(i in 1:Nsub_2){
  ...
  file_ddm <- paste0('Exp2/quantitative_fit/ddm/testfit',subs2[i],'.Rdata')
  file_conf <- paste0('Exp2/quantitative_fit/v_s_biassub/testfit',subs2[i],'.Rdata')
  file_conf_bias <- paste0('Exp2/quantitative_fit/bias/testfit',subs2[i],'.Rdata')
  file_conf_full <- paste0('Exp2/quantitative_fit/v_s_bias/testfit',subs2[i],'.Rdata')
  file_ok <- all(file.exists(file_conf,file_ddm,file_conf_bias,file_conf_full))
  if(file_ok){
    load(file_ddm)
    bound[i,] <- results_ddm$optim$bestmem[1]
...
params2 <- rbind(params2,params2_bias,params2_full)
# Generate model simulations ----------------------------------------------
rm(Simuls2)
for (s in 1:Nsub_2) {
