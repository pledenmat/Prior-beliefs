  ...
  if (length(mu)==1) {
    ...
    hm <- sapply(time_vec,function(x){
      if (x==0) {
        ...
        dist_pos <- dmnorm(matrix(rep(ev_mapping,Nv),ncol = Nv),mean=mu*x,varcov=sigma_mat*sqrt(x))
        dist_neg <- dmnorm(matrix(rep(ev_mapping,Nv),ncol = Nv),mean=-mu*x,varcov=sigma_mat*sqrt(x))
        res <- dist_pos / (dist_pos + dist_neg)
      }
      c(res)
    })
