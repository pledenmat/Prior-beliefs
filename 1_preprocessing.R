...
names(Data_exp1) <- c("sub","task","fbcond","trialdifflevel","rt","resp","cor","cj","RTconf","block")
...
names(Training_exp1) <- c("sub","task","fbcond","trialdifflevel","rt","resp","cor","cj","RTconf")
...
for (i in seq(1,dim(Training_exp1)[1],len_block)) {
  if (Training_exp1[i,"fbcond"]=="lowSC") {
    if (block%%5==1) {
  ...
  if (Training_exp1[i,"fbcond"]=="mediumSC") {
    Training_exp1[i:(i+len_block-1),]$fb <- round(runif(1,min=.69,max=.82),2)
  }

  if (Training_exp1[i,"fbcond"]=="highSC") {
    if (block%%5==2) {
